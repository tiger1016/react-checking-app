...
export default (customerId) => async dispatch => {
  try {
    dispatch({ type: FETCH_SERVICE_RATES_REQUESTED, payload: `Fetching service rates for customer ${customerId}` })
    const { data: { data: payload } } = await api.get(`/customers/${customerId}/services/rates`)
    dispatch({ type: FETCH_SERVICE_RATES_SUCCEDED, payload })
  } catch (error) {
    dispatch({ type: FETCH_SERVICE_RATES_REJECTED, payload: error.message || error.toString() })
  }
}
