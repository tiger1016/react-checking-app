...
export default (customerId) => async dispatch => {
  try {
    dispatch({ type: FETCH_ADDONS_RATES_REQUESTED, payload: `Fetching addons rates for customer ${customerId}` })
    const { data: { data: payload } } = await api.get(`/customers/${customerId}/addons/rates`)
    dispatch({ type: FETCH_ADDONS_RATES_SUCCEDED, payload })
  } catch (error) {
    dispatch({ type: FETCH_ADDONS_RATES_REJECTED, payload: error.message || error.toString() })
  }
}
